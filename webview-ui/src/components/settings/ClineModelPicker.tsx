...
const ClineModelPicker: React.FC<ClineModelPickerProps> = ({ isPopup, currentMode, showProviderRouting, initialTab }) => {
	...
	const refreshClineRecommendedModels = useCallback(async (): Promise<boolean> => {
		try {
			...
			console.error("Failed to refresh GenCoder recommended models:", error)
			return false
		}
	}, [])
		...
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			{hasInfo ? (
				...
				<p
					...
					The extension automatically fetches the latest GenCoder model list. If you're unsure which model to choose,
					Cline works best with <strong>anthropic/claude-sonnet-4.5</strong>.
				</p>
			)}
		</div>
	)
